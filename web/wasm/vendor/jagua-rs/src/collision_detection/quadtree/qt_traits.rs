...
impl QTQueryable for Edge {
    ...
    fn collides_with_quadrants(&self, r: &Rect, qs: [&Rect; 4]) -> [bool; 4] {
        ...
        // Collinear contact with a bisector may not produce a unique intersection.
        let candidates = [c_q0, c_q1, c_q2, c_q3];
        let classified =
            std::array::from_fn(|i| candidates[i].unwrap_or_else(|| self.collides_with(qs[i])));
        #[cfg(debug_assertions)]
        debug_assert!(
