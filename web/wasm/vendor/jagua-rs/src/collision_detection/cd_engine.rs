...
// A rectangular exterior has no collision with a query wholly inside its closed
// bounds. Keep every other hazard and all nonrectangular exteriors unchanged.
struct BoundaryFilter<'a, F> {
    inner: &'a F,
    skip: Option<HazKey>,
}
impl<F: HazardFilter> HazardFilter for BoundaryFilter<'_, F> {
    fn is_irrelevant(&self, key: HazKey) -> bool {
        self.skip == Some(key) || self.inner.is_irrelevant(key)
    }
}
struct BoundaryCollector<'a, C> {
    inner: &'a mut C,
    skip: Option<HazKey>,
}
impl<C: HazardCollector> HazardCollector for BoundaryCollector<'_, C> {
    fn contains_key(&self, key: HazKey) -> bool {
        self.skip == Some(key) || self.inner.contains_key(key)
    }
    fn insert(&mut self, key: HazKey, entity: HazardEntity) {
        if self.skip != Some(key) {
            self.inner.insert(key, entity);
        }
    }
    fn remove_by_key(&mut self, key: HazKey) {
...
impl CDEngine {
    fn contained_rectangular_exterior(&self, bbox: Rect) -> Option<HazKey> {
        let shape = &self.hazards_map[self.hkey_exterior].shape;
        let outer = shape.bbox;
        if bbox.x_min >= outer.x_min
            && bbox.x_max <= outer.x_max
            && bbox.y_min >= outer.y_min
            && bbox.y_max <= outer.y_max
            && shape.edge_iter().count() == 4
            && shape
                .edge_iter()
                .all(|edge| edge.start.x() == edge.end.x() || edge.start.y() == edge.end.y())
        {
            Some(self.hkey_exterior)
        } else {
            None
        }
    }
    ...
    pub fn detect_poly_collision(&self, shape: &SPolygon, filter: &impl HazardFilter) -> bool {
        let boundary_filter = BoundaryFilter {
            inner: filter,
            skip: self.contained_rectangular_exterior(shape.bbox),
        };
        let filter = &boundary_filter;
    ...
    pub fn detect_surrogate_collision(
        ...
        for pole in base_surrogate.ff_poles() {
            ...
            let boundary_filter = BoundaryFilter {
                inner: filter,
                skip: self.contained_rectangular_exterior(t_pole.bbox()),
            };
            if self.quadtree.collides(&t_pole, &boundary_filter).is_some() {
                return true;
            }
        }
        for pier in base_surrogate.ff_piers() {
            ...
            let boundary_filter = BoundaryFilter {
                inner: filter,
                skip: self.contained_rectangular_exterior(bbox),
            };
            if self.quadtree.collides(&t_pier, &boundary_filter).is_some() {
                return true;
            }
        }
        false
    ...
    {
        let mut boundary_collector = BoundaryCollector {
            inner: collector,
            skip: self.contained_rectangular_exterior(shape.bbox),
        };
        let collector = &mut boundary_collector;
        if self.bbox().relation_to(shape.bbox) != GeoRelation::Surrounding
