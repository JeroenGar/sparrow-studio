...
#[wasm_bindgen]
pub fn run(input: &str, seconds: u32, seed: &str, clearance: f32, callback: js_sys::Function) -> Result<(), JsValue> {
    console_error_panic_hook::set_once();
    let initialized_at = Instant::now();
    if !matches!(seconds, 10 | 30 | 60 | 120) || input.len() > 10 * 1024 * 1024 || !clearance.is_finite() || clearance < 0.0 {
    ...
    optimize(instance, Xoshiro256PlusPlus::seed_from_u64(seed), &mut listener,
        &mut BasicTerminator::new(), &config.expl_cfg, &config.cmpr_cfg, None);
