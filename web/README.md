...
- Browse all 34 main/Gardeyn benchmark files as complete sample projects. Original benchmarks retain their coordinates, demand and rotations. The shape library lists their reusable shape types separately, using one scale factor per source file so the median shape-type net area (outer minus holes) is 100 mm²; demand does not affect the scale.
