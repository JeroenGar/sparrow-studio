...
- The editor opens while the bundled `gardeyn2.json` example loads and prepares in the background, without automatically running the solver. The status bar shows loading progress; creating, opening, or editing a project takes precedence over the pending example. A failed example leaves the editor usable. New project starts empty. The project-name menu groups New, Open, Rename and examples; Save downloads a named `.sparrow-project.json` file, including empty projects. Replacing unsaved work offers Download and continue, Discard, or Cancel. Undo stays within the current project.
