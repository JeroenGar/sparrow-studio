...
export default function App({initialDocument=emptyProject(),initialError=''}:{initialDocument?:Document;initialError?:string}) {
  ...
  const [exportFormat,setExportFormat]=useState<'svg'|'dxf'|'zip'>('svg');
  ...
  const history=useRef<{doc:Document;geometry:boolean;selection:CopyRef[]}[]>([]),future=useRef<{doc:Document;geometry:boolean;selection:CopyRef[]}[]>([]),operation=useRef(0);
  ...
  const selected=useMemo(()=>[...new Set(selectedCopies.map(copy=>copy.partId))],[selectedCopies]);
  ...
  function restore(redo=false) {
    ...
    if(!entry) return;
    to.current.push({doc,geometry:entry.geometry,selection:selectedCopies});setDoc(withDocumentPlacements(entry.doc));
  ...
  useEffect(()=>{
    const key=(e:KeyboardEvent)=>{try {
      ...
      if(e.key==='Escape') {setSelectedCopies([]);setPolygon(undefined);}
      ...
      if(e.key==='Backspace'||e.key==='Delete') {e.preventDefault();commit(removeCopies(canvasDocument,selectedCopies));setSelectedCopies([]);return;}
      ...
      e.preventDefault();
      const parents=parts.map(part=>refs.find(copy=>copy.partId===part.id)!);
      if(delta>0) {
        const next=duplicateCopies(canvasDocument,parents);commit(next);
  ...
  async function exportLayout() {
    ...
    if(exportFormat==='zip'){await downloadArchive();return;}
    setBusy(true);setError('');
    try {
  ...
  return <div className="app" onDragOver={e=>e.preventDefault()} onDrop={e=>{e.preventDefault();if(!document.querySelector('dialog[open]'))void openFiles(e.dataTransfer.files);}}>
    ...
    <main className="main-workspace">
      <aside id="parts-settings" className={`sidebar ${panel?'open':''}`}>
        ...
        <div className="parts-list">{doc.parts.map((p,i)=>{const b=bounds(p.outer);return <div key={p.id} className={`part-row ${selected.includes(p.id)?'selected':''}`}>
          <button className="part-select" aria-pressed={selected.includes(p.id)} onClick={e=>{const next=e.shiftKey?(selected.includes(p.id)?selected.filter(id=>id!==p.id):[...selected,p.id]):selected.length===1&&selected[0]===p.id?[]:[p.id];setSelectedCopies(copyRefsFor(doc,next));}}>
      ...
      <section className="drawing-panel"><div className="view-tabs"><strong>Editable canvas</strong>{(live||result)&&<div className="result-mode" role="group" aria-label="Result display"><button aria-pressed={resultMode==='live'} disabled={!live} onClick={()=>setResultMode('live')}>Live</button><button aria-pressed={resultMode==='checked'} disabled={!result} onClick={()=>setResultMode('checked')}>Checked {result&&'✓'}</button></div>}<button className="mobile-settings" aria-expanded={panel} aria-controls="parts-settings" onClick={()=>setPanel(!panel)}>Settings</button></div>
        ...
        {showingLive&&<div className="result-details live-details"><span className="live-label">{running?'Live search':'Last live layout'}</span><span><i className="overlap-key"/>Overlaps in red</span><p>Intermediate layouts may overlap. {result?'Downloads use the best checked layout.':'Waiting for a checked layout before downloads are available.'}</p></div>}
        {solver.liveError&&<p className="field-error">Live preview unavailable: {solver.liveError}</p>}
        {result&&!showingLive&&<div className="result-details"><span className="checked">✓ Geometry checked</span><span>{maxApprox?`Curves approximated to ${displayLength(maxApprox,unit)} ${unit}`:'Polygonal contours'}</span>
        ...
        {chosen&&selectedBox&&<aside className="selection-panel" aria-label="Part properties"><div className="panel-title"><h2>Part properties</h2><button aria-label="Clear selection" onClick={()=>{setSelectedCopies([]);}}>×</button></div><section className="part-settings">{selected.length===1?<label>Name<input value={chosen.name} disabled={locked} onChange={e=>editPart({name:e.target.value},false)}/></label>:<h2>{selected.length} parts selected</h2>}
          <SelectionControls key={JSON.stringify(selectedCopies)} unit={unit} box={selectedBox} disabled={locked} onPosition={positionSelection} onSize={(axis,value)=>void transformSelection({kind:'scale',factor:value/(selectedBox[axis+2]-selectedBox[axis]),pivot:[selectedBox[0],selectedBox[1]]})} onRotate={degrees=>void transformSelection({kind:'rotate',degrees,pivot:[(selectedBox[0]+selectedBox[2])/2,(selectedBox[1]+selectedBox[3])/2]})} onValidity={setSizeValid}/>
    ...
    <footer className="statusbar"><div className="run-controls">{running?<button className="run-button" onClick={solver.stop}>Stop</button>:<button className="run-button" disabled={locked||invalidSettings||!doc.parts.some(part=>part.quantity>0)||!!polygon} onClick={()=>void run()}>{result?'Run again':'Nest parts'}</button>}</div>
      <div className="run-status"><span role="status" className="run-state"><span className="status-symbol" aria-hidden="true"><i className={running||busy?'active':undefined}/></span><span>{busy?'Checking inputs':solver.state}{running&&` · ${solver.elapsed.toFixed(1)} s`}</span></span>{solver.workers&&<small className="worker-status" title={solver.workers.reason} data-worker-count={solver.workers.actual}>{`${solver.workers.actual} solver worker${solver.workers.actual===1?'':'s'}`}{solver.workers.requested?` / ${solver.workers.requested} requested`:' · automatic'}{solver.workers.reason&&' · fallback'}</small>}</div>
