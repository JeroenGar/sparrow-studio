...
export default function App({initialDocument=emptyProject(),initialError='',loadDefaultExample=false}:{initialDocument?:Document;initialError?:string;loadDefaultExample?:boolean}) {
  ...
  async function downloadArchive() {
    ...
    try {
      ...
      download(`${exportName}.zip`,reply.archive,'application/zip');if(result)setDownloadedResult(true);setSaved({document:doc,result});
  ...
  async function saveProject() {
    ...
    try {
      ...
      download(`${exportName}.sparrow-project.json`,reply.text);if(result)setDownloadedResult(true);setSaved({document:doc,result});return true;
    }catch(e){setError(String(e));return false;}finally{setBusy(false);}
  }
  const exportName=doc.name.trim().replace(/[<>:"/\\|?*\x00-\x1f]/g,'-').replace(/[. ]+$/g,'').slice(0,100)||'project';
    ...
    <main className="main-workspace">
      ...
      <section className="drawing-panel"><div className="view-tabs"><strong>Editable canvas</strong><button className="mobile-settings" aria-expanded={panel} aria-controls="parts-settings" onClick={()=>setPanel(!panel)}>Settings</button></div>
        ...
        {chosen&&selectedBox&&<aside className="selection-panel" aria-label="Part properties"><div className="panel-title"><h2>Part properties</h2><button aria-label="Clear selection" onClick={()=>{setSelectedCopies([]);}}>×</button></div><section className="part-settings">{selected.length===1?<label>Name<input value={chosen.name} disabled={locked} onChange={e=>editPart({name:e.target.value},false)}/></label>:<h2>{selected.length} parts selected</h2>}
          ...
          <label>Permitted rotations<select disabled={locked} value={mixedRotations?'mixed':chosen.rotations.kind==='continuous'?'free':JSON.stringify(chosen.rotations.degrees)} onChange={e=>{if(e.target.value==='custom'){const text=prompt('Allowed degrees, separated by commas',chosen.rotations.kind==='discrete'?chosen.rotations.degrees.join(', '):'0, 180');if(text===null){e.currentTarget.value=mixedRotations?'mixed':chosen.rotations.kind==='continuous'?'free':JSON.stringify(chosen.rotations.degrees);return;}if(text!==null){const degrees=text.split(',').map(s=>s.trim()===''?NaN:Number(s));if(degrees.length&&degrees.every(Number.isFinite))editPart({rotations:{kind:'discrete',degrees}});else setError('Enter a nonempty list of finite degrees.');}}else editPart({rotations:e.target.value==='free'?{kind:'continuous'}:{kind:'discrete',degrees:JSON.parse(e.target.value)}});}}>
            {mixedRotations&&<option value="mixed" disabled>Mixed</option>}
