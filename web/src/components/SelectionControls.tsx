...
export default function SelectionControls({box,disabled,onPosition,onSize,onRotate,onValidity,unit='mm'}:{box:number[];disabled:boolean;onPosition:(axis:0|1,value:number)=>void;onSize:(axis:0|1,value:number)=>void;onRotate:(degrees:number)=>void;onValidity:(valid:boolean)=>void;unit?:DisplayUnit}) {
  ...
  return <><div className="size-controls">{['X','Y','Width','Height'].map((label,i)=><label key={label}>{label}, {unit}<input type="number" step="any" min={(i<2?-100000:.000001)/scale} max={100000/scale} required disabled={disabled} aria-invalid={!valid(fields[i],i)} value={fields[i]} onChange={e=>setFields(f=>f.map((v,j)=>j===i?e.target.value:v))} onBlur={()=>apply(i)} onKeyDown={e=>{if(e.key==='Enter')e.currentTarget.blur();}}/></label>)}<small>Position is for preparation. Aspect ratio locked.</small>{invalid&&<small className="field-error" role="alert">Enter finite positions and positive dimensions up to {unit==='mm'?'100,000':displayLength(100000,unit)} {unit}.</small>}</div>
