...
export async function openExamples(page:Page){await page.locator('.project-menu>summary').click();await page.getByRole('button',{name:'Load example project',exact:true}).click();}
export async function finishSwitch(page:Page){
  ...
  if(await guard.isVisible())await guard.getByRole('button',{name:'Replace without exporting',exact:true}).click();
