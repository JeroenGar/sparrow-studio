...
test('search metadata, sitemap and sharing image are available without rendering',async({request})=>{
  ...
  expect(imageUrl).toBe('https://sparrowstudio.app/sparrow-studio-preview.png');
  const image=await request.get(new URL(imageUrl).pathname);
  expect(image.ok()).toBe(true);
  expect(image.headers()['content-type']).toContain('image/png');
  const png=await image.body();
  expect(png.readUInt32BE(16)).toBe(3680);
  expect(png.readUInt32BE(20)).toBe(2228);
...
for(const javaScriptEnabled of [true,false]) {
  test.describe(javaScriptEnabled?'with editor':'without JavaScript',()=>{
    ...
    test('the introduction is collapsed, keyboard accessible and readable on desktop and mobile',async({page},testInfo)=>{
      ...
      page.on('request',request=>{if(request.url().includes('sparrow-studio-preview'))imageRequests.push(request.url());});
      await page.goto('/');
      ...
      for(const width of [1440,390]) {
        ...
        await expect(page.getByRole('heading',{name:'Free online 2D nesting with sparrow/studio'})).toBeVisible();
