...
test('project and checked result ZIP downloads include runnable CLI input',async({page},info)=>{
  ...
  await page.getByRole('button',{name:'Nest parts',exact:true}).click();await page.getByRole('button',{name:'Checked ✓',exact:true}).click({timeout:20000});await page.getByRole('button',{name:'Stop',exact:true}).click();
